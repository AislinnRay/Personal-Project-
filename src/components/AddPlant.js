...
class AddPlant extends Component {
  ...
  componentDidUpdate(prevProps) {
    const { plant_id } = this.props.match.params
    if (!plant_id && prevProps.match.params.plant_id) { // if there is no longer an id and there was previously one (need previous one so this does not fire over and over)
      this.setState({ isEdit: false, common_name: "",scientific_name: "", note: "", plant_img: "", water_interval:"" })
    }
  }
  ...
  handleAdd = () => {
    const { common_name, scientific_name, note, plant_img, water_interval} = this.state
    axios.post("/api/plants", { common_name, scientific_name, note, plant_img, water_interval }).then(() => {
  ...
  handleEdit = () => {
    const { common_name, scientific_name, note, plant_img, water_interval } = this.state
    axios
      .put(`/api/plants/${this.props.match.params.plant_id}`, { common_name, scientific_name, note, plant_img, water_interval })
      .then(() => {
        this.props.history.push("/dash")
      })
  }

  resetState = () => this.setState({ common_name: "",scientific_name: "", note: "", plant_img: "", water_interval:10});

  render() {
    const { common_name, scientific_name, note, plant_img, water_interval, isEdit } = this.state
    return (
      <div className="Form">
        <img className='form_img_preview' src={plant_img} 
